    ...
    def load_users(self):
        if os.path.exists(self.data_file):
            with open(self.data_file, 'r') as f:
                data = json.load(f)
                return {user['username']: User(user['username'], user['email'], user['password_hash']) for user in data}
        return {}

    def save_users(self):
        data = [{'username': user.username, 'email': user.email, 'password_hash': user.password_hash} for user in self.users.values()]
